    ...
    <AdminLayout>
      <div style={{ fontFamily: "'Segoe UI', Arial, sans-serif" }}>
        ...
        <header style={{ background: '#fff', padding: isMobile ? '12px 16px' : '0 24px', height: isMobile ? 'auto' : '56px', display: 'flex', alignItems: isMobile ? 'stretch' : 'center', justifyContent: 'space-between', borderBottom: '1px solid #e5e7eb', flexShrink: 0, flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? '10px' : '0' }}>
          <div style={{ fontSize: '15px', fontWeight: '600', color: '#111827' }}>{t('adminUsersTitle')}</div>
        ...
        <main style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '16px' : '20px 24px', background: '#f8fafb' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: '14px', marginBottom: '24px' }}>
            {[
              { label: t('adminActiveDrivers'), value: conducteursActifs, bg: '#dcfce7', border: '#22c55e', icon: (
              ...
              { label: t('adminInactiveDrivers'), value: conducteursInactifs, bg: '#fee2e2', border: '#dc2626', icon: (
              ...
              { label: t('adminActivePassengers'), value: passagersActifs, bg: '#dbeafe', border: '#3b82f6', icon: (
              ...
              { label: t('adminInactivePassengers'), value: passagersInactifs, bg: '#fef3c7', border: '#d97706', icon: (
          ...
          <div style={{ background: '#fff', borderRadius: '10px', border: '1px solid #e5e7eb', marginBottom: '20px', overflow: 'hidden' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '14px', fontWeight: '600', color: '#111827' }}>{t('adminDriversList')}</span>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#f9fafb' }}>
                    {[t('adminTableName'), t('adminTableEmail'), t('adminTablePhone'), t('adminTableRating'), t('adminTableStatus'), t('adminTableActions')].map(h => (
                ...
                <tbody>
                  {conducteurs.length === 0 ? (
                    <tr><td colSpan={6} style={{ padding: '30px', textAlign: 'center', color: '#9ca3af', fontSize: '13px' }}>{t('noDriversFound')}</td></tr>
                  ) : conducteurs.map((u, i) => (
                    <tr key={u.id} style={{ background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <span style={{ padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '600', background: u.estActif ? '#dcfce7' : '#fee2e2', color: u.estActif ? '#15803d' : '#dc2626' }}>
                          {u.estActif ? t('statusActive') : t('statusSuspended')}
                        </span>
                      </td>
                      <td style={{ padding: '12px 16px' }}>
          ...
          <div style={{ background: '#fff', borderRadius: '10px', border: '1px solid #e5e7eb', overflow: 'hidden' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '14px', fontWeight: '600', color: '#111827' }}>{t('adminPassengersList')}</span>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {passagers.length === 0 ? (
                    <tr><td colSpan={6} style={{ padding: '30px', textAlign: 'center', color: '#9ca3af', fontSize: '13px' }}>{t('noPassengersFound')}</td></tr>
                  ) : passagers.map((u, i) => (
                    <tr key={u.id} style={{ background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                      <td style={{ padding: '12px 16px', fontSize: '13px', color: '#111827', fontWeight: '500' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ background: '#fff', borderRadius: '14px', padding: '28px', width: '100%', maxWidth: '600px', maxHeight: '80vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <div>
                ...
                <p style={{ fontSize: '12px', color: '#6b7280', marginTop: '4px' }}>
                  {selectedConducteur?.prenom} {selectedConducteur?.nom} — {t('adminNote')}{selectedConducteur?.noteMoyenne ? `${selectedConducteur.noteMoyenne.toFixed(1)}/5` : t('adminNoRating')}
                </p>
              </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
              {loadingEval ? (
                <div style={{ textAlign: 'center', padding: '30px', color: '#6b7280' }}>{t('loading')}</div>
                ...
                <div style={{ textAlign: 'center', padding: '40px' }}>
                  ...
                  <p style={{ color: '#9ca3af', fontSize: '13px' }}>{t('noEvaluationsForDriver')}</p>
                </div>
              ) : (
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: '#f9fafb' }}>
                      {[t('adminTableAuthor'), t('adminTableRating'), t('adminTableComment'), t('date')].map(h => (
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
              ...
              <div>
                <h3 style={{ fontSize: '18px', fontWeight: '700', color: '#111827', margin: 0 }}>
                  {confirmModal.action === 'suspendre' ? t('adminSuspendUser') : t('adminReactivateUser')}
                </h3>
                <p style={{ fontSize: '13px', color: '#6b7280', margin: '4px 0 0' }}>
                  {confirmModal.action === 'suspendre'
                    ? t('adminSuspendWarning')
                    : t('adminReactivateInfo')
                  }
                </p>
              </div>
            </div>
            ...
            <div style={{
              ...
              <p style={{ fontSize: '14px', color: '#374151', margin: 0 }}>
                {t('adminConfirmQuestion')} <strong>{confirmModal.action === 'suspendre' ? t('adminSuspend') : t('adminReactivate')}</strong> <strong>{confirmModal.userName}</strong> ?
              </p>
            </div>
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              ...
              >
                {confirmModal.action === 'suspendre' ? t('adminSuspend') : t('adminReactivate')}
              </button>
            </div>
          </div>
        </div>
