...
export default function AdminProfilPage() {
  ...
  const handleUploadPhoto = async () => {
    ...
    try {
      ...
      if (jsonStart === -1 || jsonEnd === 0) throw new Error('Reponse invalide');
      ...
      if (res.ok) {
        ...
        setMessage({ type: 'error', text: data.error || t('uploadError') });
      }
    } catch (err: any) {
  ...
  return (
    <AdminLayout>
      <style>{`
        @media (max-width: 768px) { .admin-profile-grid { grid-template-columns: 1fr !important; } }
        @media (max-width: 640px) { .admin-form-grid { grid-template-columns: 1fr !important; } }
      `}</style>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        <h1 style={{ fontSize: '24px', fontWeight: '800', color: darkMode ? '#FFFFFF' : '#111827', marginBottom: '24px' }}>
          <Icon name="shield" size={24} /> {t('myProfile')}
        </h1>
        ...
        <div className="admin-profile-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '24px' }}>
          <div style={{ background: darkMode ? '#1A1A1A' : '#fff', borderRadius: '16px', border: `1px solid ${darkMode ? '#334155' : '#e5e7eb'}`, padding: '24px', textAlign: 'center' }}>
            ...
            <div style={{
              ...
              {displayPhoto ? (
                <img src={displayPhoto} alt="Apercu" style={{ width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', top: 0, left: 0, zIndex: 10 }} onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none'; }} />
              ) : null}
            ...
            <label style={{
              ...
              border: `1px solid ${darkMode ? '#334155' : '#e5e7eb'}`, borderRadius: '8px', fontSize: '13px',
            ...
            {selectedFile && (
              <button onClick={handleUploadPhoto} disabled={uploading} style={{
          ...
          <div style={{ background: darkMode ? '#1A1A1A' : '#fff', borderRadius: '16px', border: `1px solid ${darkMode ? '#334155' : '#e5e7eb'}`, padding: '32px' }}>
            ...
            <form onSubmit={handleSaveProfile}>
              <div className="admin-form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                <div>
                  ...
                  <input type="text" value={user?.prenom || ''} onChange={e => setUser({ ...user, prenom: e.target.value })} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: `1px solid ${darkMode ? '#334155' : '#d1d5db'}`, fontSize: '14px', outline: 'none', boxSizing: 'border-box', background: darkMode ? '#2D2D2D' : '#fff', color: darkMode ? '#FFFFFF' : '#111827' }} required />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: darkMode ? '#9CA3AF' : '#374151', marginBottom: '6px' }}>{t('lastName')}</label>
                  <input type="text" value={user?.nom || ''} onChange={e => setUser({ ...user, nom: e.target.value })} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: `1px solid ${darkMode ? '#334155' : '#d1d5db'}`, fontSize: '14px', outline: 'none', boxSizing: 'border-box', background: darkMode ? '#2D2D2D' : '#fff', color: darkMode ? '#FFFFFF' : '#111827' }} required />
                </div>
              </div>

              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: darkMode ? '#9CA3AF' : '#374151', marginBottom: '6px' }}>{t('email')}</label>
                <input type="email" value={user?.email || ''} disabled style={{ width: '100%', padding: '12px', borderRadius: '8px', border: `1px solid ${darkMode ? '#334155' : '#e5e7eb'}`, background: darkMode ? '#2D2D2D' : '#f9fafb', fontSize: '14px', color: darkMode ? '#6B7280' : '#6b7280', boxSizing: 'border-box' }} />
              </div>

              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: darkMode ? '#9CA3AF' : '#374151', marginBottom: '6px' }}>{t('phone')}</label>
                <input type="tel" value={user?.telephone || ''} onChange={e => setUser({ ...user, telephone: e.target.value })} placeholder={t('phonePlaceholder')} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: `1px solid ${darkMode ? '#334155' : '#d1d5db'}`, fontSize: '14px', outline: 'none', boxSizing: 'border-box', background: darkMode ? '#2D2D2D' : '#fff', color: darkMode ? '#FFFFFF' : '#111827' }} />
              </div>
              ...
              <div style={{ marginBottom: '24px' }}>
                ...
                <textarea value={user?.biographie || ''} onChange={e => setUser({ ...user, biographie: e.target.value })} placeholder={t('bioPlaceholder')} rows={4} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: `1px solid ${darkMode ? '#334155' : '#d1d5db'}`, fontSize: '14px', outline: 'none', resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit', background: darkMode ? '#2D2D2D' : '#fff', color: darkMode ? '#FFFFFF' : '#111827' }} />
              </div>

              <button type="submit" disabled={saving} style={{ width: '100%', padding: '14px', borderRadius: '10px', border: 'none', background: saving ? '#d1d5db' : `linear-gradient(135deg, ${E}, ${ED})`, color: '#fff', fontSize: '15px', fontWeight: '700', cursor: saving ? 'not-allowed' : 'pointer', boxShadow: saving ? 'none' : `0 4px 15px rgba(34,197,94,0.4)` }}>
