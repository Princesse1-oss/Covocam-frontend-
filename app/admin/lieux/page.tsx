...
export default function AdminLieuxPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setSuccess(isEdit ? t('placeUpdatedOk') : t('placeCreatedOk'));
        setTimeout(() => setSuccess(''), 3000);
        resetForm();
        fetchLieux();
      } else {
        const data = await res.json();
        setError(data.errors?.join(', ') || data.error || t('serverError'));
  ...
  const handleDelete = async (id: number, nom: string) => {
    if (!window.confirm(t('deactivatePlaceConfirm').replace('{name}', nom))) return;
    const token = localStorage.getItem('token');
    if (!token) return;
    try {
  ...
  const typeLabels: Record<string, string> = { tous: t('allPlaces'), ville: t('cityType'), quartier: t('quarterType'), adresse: t('addressType'), point_interet: t('poiType') };

  if (loading) {
    return <AdminLayout><div style={{ padding: '60px', textAlign: 'center', color: '#6b7280', fontSize: '14px' }}>{t('loading')}</div></AdminLayout>;
  }
    ...
    <AdminLayout>
      <div style={{ fontFamily: "'Segoe UI', Arial, sans-serif" }}>
        ...
        <div style={{ background: '#fff', padding: isMobile ? '12px 16px' : '0 24px', height: isMobile ? 'auto' : '56px', display: 'flex', alignItems: isMobile ? 'stretch' : 'center', justifyContent: 'space-between', borderBottom: '1px solid #e5e7eb', flexShrink: 0, flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? '10px' : '0' }}>
          <div style={{ fontSize: '15px', fontWeight: '600', color: '#111827' }}>{t('placesManage')}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
            ...
            <button onClick={() => { resetForm(); setShowForm(true); }} style={{ padding: '7px 16px', borderRadius: '8px', border: 'none', background: '#22c55e', color: '#fff', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>+ {t('addLabel')}</button>
          </div>
        </div>
        ...
        <main style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '16px' : '20px 24px', background: '#f8fafb' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: '14px', marginBottom: '20px' }}>
            {[
              { label: t('totalLabel'), value: lieux.length, color: '#0D9E7E', bg: '#E8F7F3' },
              { label: t('cityPlural'), value: lieux.filter(l => l.type === 'ville').length, color: '#2563eb', bg: '#dbeafe' },
              { label: t('quartersLabel'), value: lieux.filter(l => l.type === 'quartier').length, color: '#9333ea', bg: '#f3e8ff' },
              { label: t('poiPlural'), value: lieux.filter(l => l.type === 'point_interet').length, color: '#ea580c', bg: '#fff7ed' },
            ].map((s, i) => (
              ...
              <div style={{ background: '#fff', borderRadius: '14px', padding: '28px', width: '100%', maxWidth: '520px', maxHeight: '85vh', overflowY: 'auto' }}>
                ...
                <form onSubmit={handleSubmit}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '14px' }}>
                    ...
                    <div>
                      <label style={{ fontSize: '12px', fontWeight: '600', color: '#374151', display: 'block', marginBottom: '4px' }}>{t('typeRequired')}</label>
                      <select required value={form.type} onChange={e => setForm(p => ({ ...p, type: e.target.value }))} style={{ width: '100%', padding: '8px 12px', border: '1px solid #e5e7eb', borderRadius: '8px', fontSize: '13px', boxSizing: 'border-box' }}>
                        <option value="ville">{t('cityType')}</option>
                        <option value="quartier">{t('quarterType')}</option>
                        <option value="adresse">{t('addressType')}</option>
                        <option value="point_interet">{t('poiType')}</option>
                      </select>
                    </div>
                    <div>
                      <label style={{ fontSize: '12px', fontWeight: '600', color: '#374151', display: 'block', marginBottom: '4px' }}>{t('countryLabel')}</label>
                    ...
                    <div style={{ gridColumn: '1 / -1' }}>
                      <label style={{ fontSize: '12px', fontWeight: '600', color: '#374151', display: 'block', marginBottom: '4px' }}>{t('addressLabel')}</label>
                    ...
                    <div>
                      <label style={{ fontSize: '12px', fontWeight: '600', color: '#374151', display: 'block', marginBottom: '4px' }}>{t('latitudeLabel')}</label>
                    ...
                    <div>
                      <label style={{ fontSize: '12px', fontWeight: '600', color: '#374151', display: 'block', marginBottom: '4px' }}>{t('longitudeLabel')}</label>
                    ...
                    <div>
                      <label style={{ fontSize: '12px', fontWeight: '600', color: '#374151', display: 'block', marginBottom: '4px' }}>{t('postalCodeLabel')}</label>
                    ...
                    <div>
                      <label style={{ fontSize: '12px', fontWeight: '600', color: '#374151', display: 'block', marginBottom: '4px' }}>{t('regionLabel')}</label>
                    ...
                    <div style={{ gridColumn: '1 / -1' }}>
                      <label style={{ fontSize: '12px', fontWeight: '600', color: '#374151', display: 'block', marginBottom: '4px' }}>{t('descriptionLabel')}</label>
                  ...
                  <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '20px' }}>
                    <button type="button" onClick={resetForm} style={{ padding: '8px 18px', borderRadius: '8px', border: '1px solid #e5e7eb', background: '#fff', color: '#374151', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>{t('cancel')}</button>
                    <button type="submit" style={{ padding: '8px 18px', borderRadius: '8px', border: 'none', background: '#22c55e', color: '#fff', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>{editingLieu ? t('editLabel') : t('createLabel')}</button>
                  </div>
                </form>
              </div>
            </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#f9fafb' }}>
                    {['#', t('adminTableName'), t('typeHeader'), t('countryLabel'), t('regionLabel'), t('gpsLabel'), t('statusLabel')].map(h => (
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr><td colSpan={7} style={{ padding: '40px', textAlign: 'center', color: '#9ca3af', fontSize: '13px' }}>{t('noPlacesFound')}</td></tr>
                  ) : filtered.map((lieu, i) => (
                      ...
                      <td style={{ padding: '12px 14px' }}>
                        <div style={{ display: 'flex', gap: '6px' }}>
                          <span style={{ padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '600', background: lieu.estActif ? '#dcfce7' : '#fee2e2', color: lieu.estActif ? '#15803d' : '#dc2626' }}>
                            {lieu.estActif ? t('statusActive') : t('inactiveLabel')}
                          </span>
                          <button onClick={() => handleEdit(lieu)} style={{ padding: '4px 10px', background: '#e5e7eb', color: '#374151', border: 'none', borderRadius: '6px', fontSize: '11px', cursor: 'pointer', fontWeight: '600' }}>{t('editLabel')}</button>
                          <button onClick={() => handleDelete(lieu.id, lieu.nom)} style={{ padding: '4px 10px', background: '#fee2e2', color: '#dc2626', border: '1px solid #fca5a5', borderRadius: '6px', fontSize: '11px', cursor: 'pointer', fontWeight: '600' }}>{t('deactivateLabel')}</button>
                        </div>
                      </td>
                    </tr>
                  ))}
